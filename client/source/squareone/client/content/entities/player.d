module squareone.client.content.entities.player;
...
@safe class PlayerMovementScript : Script
{
	...
	override void execute()
	{
		...
		if(pc.allowInput && input.hideCursor)
		{
			...
			pc.headRotation.x += cast(float)cursorMovement.y * cast(float)entityManager.moxane.deltaTime * pc.headMovementSpeed;
			pc.headRotation.y += cast(float)cursorMovement.x * cast(float)entityManager.moxane.deltaTime * pc.headMovementSpeed;
