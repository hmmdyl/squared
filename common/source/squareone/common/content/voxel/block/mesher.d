module squareone.common.content.voxel.block.mesher;
...
final class Mesher : IMesher
{
	...
	this(BlockProcessorBase processor, VoxelRegistry resources, MeshBufferHost meshBufferHost, IChannel!MeshOrder source) @trusted
	...
	void kick() @trusted { send(thread, true); }
	void terminate() @trusted { send(thread, false); }
}

private void worker(shared BlockProcessorBase processor_, shared VoxelRegistry resources_, shared MeshBufferHost host_, shared IChannel!MeshOrder source_,
					shared bool* parked, shared bool* terminated, shared float* averageMeshTime) @trusted
{
	BlockProcessorBase processor = cast(BlockProcessorBase)processor_;
	VoxelRegistry resources = cast(VoxelRegistry)resources_;
	...
	enum threadName = BlockProcessorBase.stringof ~ " " ~ Mesher.stringof;
...
private void operateOnChunk(MeshOrder order, MeshBuffer buffer, VoxelRegistry resources, MeshBufferHost host, Log log, Object uploadSyncObj, DList!(BlockProcessorBase.MeshResult)* uploadQueue, shared float* averageMeshTime_)
{
	...
	if(buffer.vertexCount == 0)
	{
		...
		BlockProcessorBase.MeshResult mr;
		mr.order = order;
		mr.buffer = null;
